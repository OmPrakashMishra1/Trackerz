...
export async function signOut() {
    // Standard Supabase Server Client setup is asynchronous
    const supabase = await createClient(); 
    
    await supabase.auth.signOut();
    
    // Clear the cache for the home page so the UI updates
    revalidatePath("/");
...
export async function addProduct(formData) {
  const url = formData.get("url"); //taking url from form data
...
//checking is user is logged in or not
 ...
 // Validate extracted data
    ...
    //renaming existingProduct to avoid confusion
    const { data: existingProduct } = await supabase
      .from("products")
      .select("id, current_price")
      .eq("user_id", user.id) //equals to user id
      .eq("url", url)//equals to url
      .single();//single record

    const isUpdate = !!existingProduct;
    //if true then its an update else new product
